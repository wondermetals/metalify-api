...
$fuelLiveCacheFile = __DIR__ . '/fuel_live_cache.json';
$fuelLocationCacheFile = __DIR__ . '/fuel_location_cache.json';
$fuelKeyStateFile = __DIR__ . '/fuel_key_state.json';
$cacheSeconds = 8;
$fuelCacheSeconds = 300;
$fuelLocationCacheSeconds = 86400;
$configPath = __DIR__ . '/config.php';
$privateConfigPaths = [__DIR__ . '/metalify-private-config.php', dirname(__DIR__) . '/metalify-private-config.php', __DIR__ . '/../metalify-private-config.php', $configPath];
foreach ($privateConfigPaths as $candidateConfigPath) {
    if (file_exists($candidateConfigPath)) { require_once $candidateConfigPath; break; }
}
$fuelApiKeys = defined('INDIANAPI_KEYS') ? array_values(array_filter(INDIANAPI_KEYS)) : [];
if (!$fuelApiKeys) {
    $singleFuelKey = defined('INDIANAPI_KEY') ? INDIANAPI_KEY : (getenv('INDIANAPI_KEY') ?: '');
    if ($singleFuelKey !== '') $fuelApiKeys = [$singleFuelKey];
}
...
function fetchText($url, $timeout = 15) {
    if (!is_string($url) || trim($url) === '') {
        throw new Exception('Remote API URL is empty.');
    }
...
function fetchJsonWithHeaders($url, $headers = [], $timeout = 15) {
    ...
    if (function_exists('curl_init')) {
        ...
        curl_close($ch);
        if ($body === false || $status >= 400) {
            if ($status === 429) throw new Exception('IndianAPI rate limit reached.', 429);
            throw new Exception($error ?: "HTTP $status while fetching $url");
        }
        $json = json_decode($body, true);
        if (!is_array($json)) throw new Exception('Invalid JSON response.');
        $errorText = strtolower((string)($json['message'] ?? $json['error'] ?? $json['detail'] ?? ''));
        if (strpos($errorText, '429') !== false || strpos($errorText, 'rate limit') !== false || strpos($errorText, 'monthly limit') !== false || strpos($errorText, 'credits exhausted') !== false) {
            throw new Exception('IndianAPI rate limit reached.', 429);
        }
        return $json;
    }
...
function fetchFuelJsonWithRotation($url, $keys, $stateFile, $timeout = 15) {
    if (!$keys) throw new Exception('Fuel API is not configured. Upload config.php beside api.php.', 503);
    $state = readCacheFile($stateFile);
    $start = isset($state['active_index']) ? (int)$state['active_index'] : 0;
    $count = count($keys);
    for ($attempt = 0; $attempt < $count; $attempt++) {
        $index = ($start + $attempt) % $count;
        try {
            $data = fetchJsonWithHeaders($url, ['x-api-key: ' . $keys[$index]], $timeout);
            writeCacheFile($stateFile, ['active_index' => $index]);
            return $data;
        } catch (Throwable $error) {
            if ((int)$error->getCode() !== 429) throw $error;
            writeCacheFile($stateFile, ['active_index' => ($index + 1) % $count]);
        }
    }
    throw new Exception('All configured IndianAPI keys have reached their monthly request limit.', 429);
}

function findFuelCityRow($rows, $city) {
    $needle = fuelSlug($city);
    foreach ($rows as $row) {
        if (!is_array($row)) continue;
        $name = $row['city'] ?? $row['cityName'] ?? $row['name'] ?? $row['location'] ?? '';
        if ($name !== '' && fuelSlug($name) === $needle) return $row;
    }
    return null;
}

function fuelSlug($name) {
    $slug = strtolower(trim(preg_replace('/[^a-zA-Z0-9]+/', '-', $name)));
    return trim($slug, '-');
}

function getCitiesForFuelState($stateName) {
    global $fuelLocationCacheFile, $fuelLocationCacheSeconds;
    $cache = readCacheFile($fuelLocationCacheFile);
    $cacheFresh = $cache && isset($cache['generated_at']) && (time() - strtotime($cache['generated_at']) < $fuelLocationCacheSeconds);
    $states = $cacheFresh && isset($cache['states']) ? $cache['states'] : fetchJson('https://hapilocations.web.app/states/101.min.json', 10);
    if (!$cacheFresh) $cache = ['generated_at' => date('c'), 'states' => $states, 'cities' => []];
    $needle = strtolower(trim(preg_replace('/\s+/', ' ', $stateName)));
    $state = null;
    foreach ($states as $item) {
        $candidate = strtolower(trim(preg_replace('/\s+/', ' ', $item['name'] ?? '')));
        if ($candidate === $needle) { $state = $item; break; }
    }
    if (!$state || !isset($state['id'])) throw new Exception('State location data was not found.');
    $stateId = (string)$state['id'];
    if (isset($cache['cities'][$stateId]) && is_array($cache['cities'][$stateId])) return $cache['cities'][$stateId];
    $cities = fetchJson('https://hapilocations.web.app/cities/' . rawurlencode($stateId) . '.min.json', 15);
    $mappedCities = array_map(function($city) {
        return ['name' => $city['name'], 'value' => fuelSlug($city['name'])];
    }, $cities);
    $cache['cities'][$stateId] = $mappedCities;
    writeCacheFile($fuelLocationCacheFile, $cache);
    return $mappedCities;
}

function handleExtraApiRequest() {
    global $fuelApiKeys, $fuelKeyStateFile, $fuelLiveCacheFile, $fuelCacheSeconds, $fuelLocationCacheFile, $fuelLocationCacheSeconds;
    ...
    if (in_array($action, ['fuel-states', 'fuel-cities', 'fuel-rates'], true)) {
        ...
        if ($action === 'fuel-states') {
            $locationCache = readCacheFile($fuelLocationCacheFile);
            $locationFresh = $locationCache && isset($locationCache['generated_at'], $locationCache['states']) && (time() - strtotime($locationCache['generated_at']) < $fuelLocationCacheSeconds);
            $locationStates = $locationFresh ? $locationCache['states'] : fetchJson('https://hapilocations.web.app/states/101.min.json', 10);
            if (!$locationFresh) writeCacheFile($fuelLocationCacheFile, ['generated_at' => date('c'), 'states' => $locationStates, 'cities' => []]);
            $data = array_map(function($state) {
                return ['name' => $state['name'], 'value' => fuelSlug($state['name']), 'stateId' => $state['id']];
            }, $locationStates);
            ...
            $data = getCitiesForFuelState($state);
        } else {
            $city = strtolower(trim($_GET['city'] ?? ''));
            if (!preg_match('/^[a-z0-9-]+$/', $city)) respond(['error' => 'Invalid city.'], 400);
            $fuelCache = readCacheFile($fuelLiveCacheFile);
            $fuelCacheFresh = $fuelCache && isset($fuelCache['generated_at'], $fuelCache['petrol'], $fuelCache['diesel']) && (time() - strtotime($fuelCache['generated_at']) < $fuelCacheSeconds);
            if ($fuelCacheFresh) {
                $petrolRows = $fuelCache['petrol'];
                $dieselRows = $fuelCache['diesel'];
            } else {
                $petrolRows = fetchFuelJsonWithRotation($base . '/live_fuel_price?fuel_type=petrol&location_type=city', $fuelApiKeys, $fuelKeyStateFile, 15);
                $dieselRows = fetchFuelJsonWithRotation($base . '/live_fuel_price?fuel_type=diesel&location_type=city', $fuelApiKeys, $fuelKeyStateFile, 15);
                writeCacheFile($fuelLiveCacheFile, ['generated_at' => date('c'), 'petrol' => $petrolRows, 'diesel' => $dieselRows]);
            }
            ...
            $data = ['cityName' => $petrol['city'] ?? $petrol['cityName'] ?? $diesel['city'] ?? $diesel['cityName'] ?? $city, 'fuel' => [
                'petrol' => $petrol ? ['retailPrice' => (float)($petrol['price'] ?? $petrol['retailPrice'] ?? 0), 'retailUnit' => 'litre', 'change' => (float)($petrol['change'] ?? 0)] : null,
                'diesel' => $diesel ? ['retailPrice' => (float)($diesel['price'] ?? $diesel['retailPrice'] ?? 0), 'retailUnit' => 'litre', 'change' => (float)($diesel['change'] ?? 0)] : null,
                'cng' => null
            ], 'source' => 'IndianAPI'];
        }
        respond($data);
