...
def get_cities_for_fuel_state(state_name):
    states = fetch_json("https://hapilocations.web.app/states/101.min.json", 10)
    ...
    cities = fetch_json(f"https://hapilocations.web.app/cities/{state['id']}.min.json", 15)
    return [{"name": city["name"], "value": fuel_slug(city["name"])} for city in cities]
    ...
    if action.startswith("fuel-"):
        ...
        if action == "fuel-states":
            return [{"name": item["name"], "value": fuel_slug(item["name"]), "stateId": item["id"]}
                    for item in fetch_json("https://hapilocations.web.app/states/101.min.json", 10)]
